...
Micronaut deploys fine as well on App Engine. I [wrote about it](https://glaforge.appspot.com/article/getting-started-with-micronaut-on-google-app-engine-java-11) in the past already. If you're using Micronaut Launch, just select the Application type. App Engine allows you to deploy the standalone runnable JARs generated by the configured shadow JAR plugin. But if you want to easily stage your application deliverable, to run the application locally, to deploy, you can also use the Gradle App Engine plugin.
